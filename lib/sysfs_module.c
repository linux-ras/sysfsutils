...
/**
 * sysfs_open_module_attr: read an attribute for a given module
 * @module: name of the module on which to look
		...
		const char *attrib)
{
	...
	safestrcat(path, "/");
	safestrcat(path, module);
	safestrcat(path, "/");
	safestrcat(path, attrib);
	attribute = sysfs_open_attribute(path);
	if (attribute == NULL) {
		dprintf("Error opening attribute %s on module %s\n",
				attrib, module);
		return NULL;
	}
	if ((sysfs_read_attribute(attribute)) != 0) {
		dprintf("Error reading attribute %s for module %s\n",
				attrib, module);
